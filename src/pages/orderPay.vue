<template>
  <div class="order-pay">
    <order-header title="订单支付">
      <template v-slot:tip>
        <span>请谨防钓鱼链接或诈骗电话，了解更多888</span>
      </template>
    </order-header>
    <div class="wrapper">
      <div class="container">
    ...
    <scan-pay-code v-if="showPay" @close="close" :img="payImg"></scan-pay-code>
    <modal
  ...
  methods: {
    getOrderDetail() {
      this.axios.get(`/orders/${this.orderId}`).then((res) => {
        ...
        // console.log(item)
        this.addressInfo = `${item.receiverProvince} ${item.receiverCity} ${item.receiverDistrict} ${item.receiverAddress}`
